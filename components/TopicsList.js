    ...
    <>
        <div className="flex">
          {inputStates.map((isChecked, index) => (
            <div key={index} className="flex items-center mr-4">
              ...
              <label for="inline-radio" className="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300">{categoryName[index]}</label>
            </div>
          ))}
        </div>   
        {postingData.map((elem,index)=>{
          ...
          <div key={index} className="flex w-full items-center bg-white border border-gray-200 rounded-lg shadow ">
            ...
            <RemoveBtn/>
            <Link href={`/admin/edit/${categoryIndex}_${no}`}>
